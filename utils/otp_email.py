...
def _get_timeout_seconds() -> float:
    ...
    try:
        ...
        value = 8.0
    # Keep it reasonable so web requests don't hang.
    if value <= 0:
        return 8.0
...
def _send_via_sendgrid_http(
    *,
    ...
    try:
        ...
        # Return a short error to avoid leaking too much provider detail.
        try:
            body = e.read(512)
            extra = body.decode('utf-8', errors='ignore').strip()
        except Exception:
            extra = ''
